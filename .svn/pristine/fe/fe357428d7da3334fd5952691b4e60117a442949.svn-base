			...
			<ul class="nav navbar-nav navbar-right">
				...
				<c:if test="${user ne null}">
					<li class="navbar-li-right"><a href="editMessage.jsp"
						style="color: white;"><%=user.getUser_nike()%></a></li>
				...
				<div style="text-align: right; margin-top: 10px;">
					<div class="btn-group btn-group-lg">
						<a href="searchall.do"><button type="button"
								...
								style="background-color: #34ce6c; color: #ffffff;">按距离排行</button></a>
						<a href="searchbyeva.do"><button type="button"
								class="btn btn-default" id="btn-choose">按好评度排行</button></a>
					</div>
				</div>
				<!--数据库检索循环罗列出信息排行-->
					...
					<c:choose>
						<c:when
							...
							<button type="button" class="btn btn-default "
								disabled="disabled">上一页</button>&nbsp;&nbsp; |<%=pd.getPageindex()%>| &nbsp;&nbsp;下一页
						...
						<c:when test="${pagedata.getPageindex()==1 }">
							...
							<button type="button" class="btn btn-default" disabled="disabled">上一页</button>&nbsp;&nbsp;|<%=pd.getPageindex()%>|&nbsp;&nbsp;<a
								href="searchall.do?currentpage=<%=pd.getPageindex() + 1%>" style="text-decoration:none">
						...
						<c:when test="${pagedata.getPageindex()==pagedata.getPageCount()}">
							...
							<a href="searchall.do?currentpage=<%=pd.getPageindex() - 1%>"><button
